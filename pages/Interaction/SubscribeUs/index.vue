<template>
  <div class="page-wrap">
    ...
    <div class="filter-bar">
      <a-input-search v-model="keyword" placeholder="Tìm kiếm email..." style="width:280px" @search="fetchList" allow-clear />
    </div>
    <a-table :columns="columns" :data-source="list" :loading="loading" :pagination="pagination" row-key="mailNotificationId" @change="onTableChange">
      <template #action="record">
        <a-popconfirm title="Xóa email này?" ok-text="Xóa" ok-type="danger" cancel-text="Hủy" @confirm="remove(record)">
...
<script>
import { getListSendSubcribeUs, deleteSendMailSubscribeUs } from "../../../apis/interaction";
...
export default {
  ...
  methods: {
    async fetchList() {
      this.loading = true;
      try {
        const res = await getListSendSubcribeUs({ keyWord: this.keyword || undefined, currentPage: this.page, recordPerPage: this.pageSize });
        if (res && res.data) {
    ...
    async remove(record) {
      try {
        await deleteSendMailSubscribeUs(record.mailNotificationId);
